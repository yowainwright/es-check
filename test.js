...
it('🎉  Es Check should pass when checking an array of es5 files as es5', (done) => {
	exec(
		'node index.js es5 ./tests/es5.js ./tests/es5-2.js',
		(err, stdout, stderr) => {
			if (err) {
...
it('🎉  Es Check should pass when checking a file with a hash bang', (done) => {
	exec(
		'node index.js es6 ./tests/scripts/hash-bang.js --allow-hash-bang',
		(err, stdout, stderr) => {
			if (err) {
				console.error(err.stack)
				console.error(stdout.toString())
...
it('👌  Es Check should fail when checking an array of es6 files as es5', (done) => {
	exec(
		'node index.js es5 ./tests/es6.js ./tests/es6-2.js',
		(err, stdout, stderr) => {
			assert(err)
			console.log(stdout)
			done()
...
it('🎉  Es Check should pass when checking a glob of es6 modules using the --module flag', (done) => {
	exec(
		'node index.js es6 ./tests/modules/*.js --module',
		(err, stdout, stderr) => {
			assert(stdout)
			if (err) {
				console.error(err.stack)
...
describe('Es Check skips folders and files included in the not flag', () => {
	it('👌  non-glob', (done) => {
		exec(
			'node index.js es5 ./tests/es5.js ./tests/modules/* --not=./tests/modules',
			(err, stdout, stderr) => {
				if (err) {
					console.error(err.stack)
					console.error(stdout.toString())
	...
	it('👌  .escheckrc', (done) => {
		exec(
			'node index.js es5 ./tests/es5.js ./tests/skipped/es6-skipped.js',
			(err, stdout, stderr) => {
				if (err) {
					console.error(err.stack)
					console.error(stdout.toString())
