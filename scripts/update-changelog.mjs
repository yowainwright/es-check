...
async function updateChangelog(version = null) {
  ...
  if (changelogContent.includes(`## [${targetVersion}]`) && changelogContent.includes('**What\'s Changed:**')) {
  ...
  if (action === 'todo') {
    ...
    const versionIndex = lines.findIndex(line => line.includes(`## [${targetVersion}]`));
    ...
    runGitCommand(`git commit -m "chore: add TODO for changelog v${targetVersion}"`);
    
    console.log(`\n📝 TODO added to changelog on branch ${changelogBranch}`);
    console.log(`   Run 'git checkout ${changelogBranch}' later to complete it.`);
