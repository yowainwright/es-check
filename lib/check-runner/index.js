...
function processConfig(config, context) {
  ...
  const featureDetectionPlan =
    config.checkFeatures && !config.checkForPolyfills && !isDebug
      ? require("../detectFeatures").createFeatureDetectionPlan(
  ...
  const errors = processBatchedFiles(filteredFiles, processFile, batchSize, {
