...
function parseFilePatterns(configFilesValue) {
  ...
  if (isString) {
    return configFilesValue.split(",").map((p) => p.trim()).filter(Boolean);
  }
...
function handleMissingFiles(pattern, options) {
  const { logger, isNodeAPI, allErrors, looseGlobMatching } = options;
  const hasLogger = logger !== null && logger !== undefined;
  const isExiting = !isNodeAPI;
...
function determineEcmaVersion(config, options) {
  ...
  if (isBrowserslistCheck) {
    ...
    if (hasError) {
      handleBrowserslistError(browserslistError, { logger, isNodeAPI, allErrors });
...
function processFullAST(code, acornOpts, file, config, ignoreList, ecmaVersion, isDebug, logger) {
  ...
  const hasReduction = isDebug && filteredUnsupportedFeatures.length !== unsupportedFeatures.length;
...
async function processConfigResult(errors, logger, isNodeAPI, allErrors, ecmaVersion) {
