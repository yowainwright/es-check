...
function findFiles(patterns, options) {
  ...
  const uniqueMatchedFiles = Array.from(new Set(allMatchedFiles));
  const noMatchedFiles = uniqueMatchedFiles.length === 0;
  ...
  return { files: uniqueMatchedFiles, hasError: hasPatternWithNoFiles };
}
...
function filterIgnoredFiles(files, pathsToIgnore) {
  const hasNoIgnores = pathsToIgnore.length === 0;
  if (hasNoIgnores) return files;

  const ignoredMatchers = pathsToIgnore.map((path) => {
    const hasWildcard = path.includes("*") || path.includes("?");
    return hasWildcard ? globPatternToRegExp(path) : path;
  });

  return files.filter((filePath) => {
    const normalizedFilePath = filePath.replace(/\\/g, "/");
    return !ignoredMatchers.some((ignoreValue) => {
      if (ignoreValue instanceof RegExp) {
        return ignoreValue.test(normalizedFilePath);
      }
      return normalizedFilePath.includes(ignoreValue.replace(/\\/g, "/"));
    });
  });
}
  ...
  try {
    const result = detectFeatures(
      ...
      ignoreList,
      {
        ...
        onlyUnsupportedFeatures: !isDebug,
        featureDetectionPlan,
      },
    );
