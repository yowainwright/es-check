...
function buildConfig(ecmaVersionArg, filesArg, options, baseConfig) {
  ...
  const ignoreValue =
    options.ignore !== undefined ? options.ignore : baseConfig.ignore;
  const ignoreFileValue =
    ignoreFilePath !== undefined ? ignoreFilePath : baseConfig.ignoreFile;
  const notValue = options.not
    ? options.not
        .split(",")
        .map((n) => n.trim())
        .filter(Boolean)
  ...
  const config = {
    ...
    browserslistQuery: browserslistQueryValue,
    browserslistPath: browserslistPathValue,
    browserslistEnv: browserslistEnvValue,
