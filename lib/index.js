...
const fs = require('fs')
const { detectFeatures, polyfillDetector: polyfillDetectorModule } = require('./helpers');
let polyfillDetector = null;
const pkg = require('../package.json')
const { lilconfig } = require('lilconfig');
const { JS_VERSIONS } = require('./constants');
const { parseIgnoreList, createLogger, generateBashCompletion, generateZshCompletion, processBatchedFiles, readFileAsync, clearFileCache, getFileCacheStats, parseCode, determineInvocationType, determineLogLevel, handleESVersionError } = require('./helpers');
  ...
  for (const config of configs) {
    ...
    const processFile = async (file) => {
      ...
      if (lightMode) {
        ...
        const { error: parseError } = parseLightMode(code, ecmaVersion, esmodule, allowHashBang, file);
        if (parseError) {
          if (isDebug) {
