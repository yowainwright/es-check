...
program
  ...
  .action((shell) => {
    ...
    switch (shell) {
      ...
      default:
        logger.error(
          `Shell "${shell}" not supported for completion. Supported shells: bash, zsh`,
        );
        process.exit(1);
    }
...
program
  ...
  .addOption(
    new Option("--no-color", "disable use of colors in output")
      .default(false)
      .hideHelp(),
  )
  ...
  .option(
    "--looseGlobMatching",
    "doesn't fail if no files are found in some globs/files",
    false,
  )
  ...
  .addOption(
    new Option(
      "--checkBrowser",
      "use browserslist configuration to determine ES version, use checkBrowser argument instead of ecmaVersion",
      false,
    ).hideHelp(),
  )
  ...
  .option(
    ...
    "path to custom browserslist configuration",
  )
  .option("--browserslistEnv <env>", "browserslist environment to use")
  .option("--config <path>", "path to custom .escheckrc config file")
  .option(
    "--batchSize <number>",
    "number of files to process concurrently (0 for unlimited)",
    "0",
  )
  .option(
    "--noCache",
    "disable file caching (caching is enabled by default)",
    false,
  )
  .option(
    "--light",
  ...
  try {
    if (customConfigPath) {
      try {
        ...
        throw new Error(
          `Error loading custom config file ${customConfigPath}: ${err.message}`,
        );
      }
    }
...
program.action(async (ecmaVersionArg, filesArg, options) => {
  ...
  if (
    ...
    ecmaVersionArg &&
    !validEcmaVersionValues.has(ecmaVersionArg)
  ) {
  ...
  if (ecmaVersionArg || filesArg?.length || options.files) {
    ...
    if (filesArg?.length) {
      ...
      singleConfig.files = options.files
        .split(",")
        .map((f) => f.trim())
        .filter(Boolean);
    }
  ...
  for (const config of configs) {
    ...
    if (configFilesValue) {
      if (Array.isArray(configFilesValue)) {
        patternsToGlob = configFilesValue
          .map((p) => String(p).trim())
          .filter(Boolean);
    ...
    patternsToGlob.forEach((pattern) => {
      ...
      if (shouldErrorOnNoFiles) {
        ...
        if (!isNodeAPI) {
          ...
          allErrors.push({
            err: new Error(
              `Did not find any files to check for pattern: ${pattern}`,
            ),
            file: "glob",
          });
          hasErrors = true;
    ...
    if (noMatchedFiles) {
      if (shouldErrorOnNoMatchedFiles) {
        if (logger)
          logger.error(
            `ES-Check: Did not find any files to check across all patterns: ${patternsToGlob.join(", ")}.`,
          );
        if (!isNodeAPI) {
          process.exit(1);
        } else {
          allErrors.push({
            err: new Error(
              `Did not find any files to check across all patterns: ${patternsToGlob.join(", ")}`,
            ),
            file: "glob",
          });
          hasErrors = true;
    ...
    const isBrowserslistCheck = Boolean(
      expectedEcmaVersion === "checkBrowser" || checkBrowser !== undefined,
    );
    ...
    if (isBrowserslistCheck) {
      ...
      try {
        ...
        if (logger)
          logger.error(
            `Error determining ES version from browserslist: ${err.message}`,
          );
        if (!isNodeAPI) {
          process.exit(1);
        } else {
      ...
      switch (expectedEcmaVersion) {
        ...
        case "es4":
          ...
          if (!isNodeAPI) {
            ...
            allErrors.push({
              err: new Error("ES4 is not supported"),
              file: "config",
            });
            hasErrors = true;
            continue;
    ...
    if (isDebug) {
      logger.debug(
        `ES-Check: Going to check files using version ${ecmaVersion}`,
      );
    }
    ...
    const processFile = async (file) => {
      ...
      if (checkForPolyfills && unsupportedFeatures.length > 0) {
        ...
        filteredUnsupportedFeatures = polyfillDetector.filterPolyfilled(
          unsupportedFeatures,
          polyfills,
        );
