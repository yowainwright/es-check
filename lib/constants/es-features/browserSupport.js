// Generated by scripts/generate/browser-support/index.mjs from @mdn/browser-compat-data 8.1.0.
// Values are the earliest browser version that ships the feature (`true` = supported, version unknown;
// `false` = not supported). Run `pnpm scripts:generate:browser-support` to refresh.
const FEATURE_BROWSER_SUPPORT = {
  AggregateError: {
    chrome: "85",
    chrome_android: "85",
    deno: "1.2",
    edge: "85",
    firefox: "79",
    firefox_android: "79",
    ie: false,
    nodejs: "15.0.0",
    opera: "71",
    opera_android: "60",
    safari: "14",
    safari_ios: "14",
    samsunginternet_android: "14.0",
    webview_android: "85",
  },
  Array: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "4",
    nodejs: "0.10.0",
    opera: "4",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "1",
  },
  ArrayBuffer: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  ArrayBufferTransfer: {
    chrome: "114",
    chrome_android: "114",
    deno: "1.33",
    edge: "114",
    firefox: "122",
    firefox_android: "122",
    ie: false,
    nodejs: "21.0.0",
    opera: "100",
    opera_android: "76",
    safari: "17.4",
    safari_ios: "17.4",
    samsunginternet_android: "23.0",
    webview_android: "114",
  },
  ArrayBufferTransferToFixedLength: {
    chrome: "114",
    chrome_android: "114",
    deno: "1.33",
    edge: "114",
    firefox: "122",
    firefox_android: "122",
    ie: false,
    nodejs: "21.0.0",
    opera: "100",
    opera_android: "76",
    safari: "17.4",
    safari_ios: "17.4",
    samsunginternet_android: "23.0",
    webview_android: "114",
  },
  ArrayFindLast: {
    chrome: "97",
    chrome_android: "97",
    deno: "1.16",
    edge: "97",
    firefox: "104",
    firefox_android: "104",
    ie: false,
    nodejs: "18.0.0",
    opera: "83",
    opera_android: "68",
    safari: "15.4",
    safari_ios: "15.4",
    samsunginternet_android: "18.0",
    webview_android: "97",
  },
  ArrayFindLastIndex: {
    chrome: "97",
    chrome_android: "97",
    deno: "1.16",
    edge: "97",
    firefox: "104",
    firefox_android: "104",
    ie: false,
    nodejs: "18.0.0",
    opera: "83",
    opera_android: "68",
    safari: "15.4",
    safari_ios: "15.4",
    samsunginternet_android: "18.0",
    webview_android: "97",
  },
  ArrayFlat: {
    chrome: "69",
    chrome_android: "69",
    deno: "1.0",
    edge: "79",
    firefox: "62",
    firefox_android: "62",
    ie: false,
    nodejs: "11.0.0",
    opera: "56",
    opera_android: "48",
    safari: "12",
    safari_ios: "12",
    samsunginternet_android: "10.0",
    webview_android: "69",
  },
  ArrayFlatMap: {
    chrome: "69",
    chrome_android: "69",
    deno: "1.0",
    edge: "79",
    firefox: "62",
    firefox_android: "62",
    ie: false,
    nodejs: "11.0.0",
    opera: "56",
    opera_android: "48",
    safari: "12",
    safari_ios: "12",
    samsunginternet_android: "10.0",
    webview_android: "69",
  },
  ArrayFromAsync: {
    chrome: "121",
    chrome_android: "121",
    deno: "1.38",
    edge: "121",
    firefox: "115",
    firefox_android: "115",
    ie: false,
    nodejs: "22.0.0",
    opera: "107",
    opera_android: "81",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "25.0",
    webview_android: "121",
  },
  ArrayPrototypeAt: {
    chrome: "92",
    chrome_android: "92",
    deno: "1.12",
    edge: "92",
    firefox: "90",
    firefox_android: "90",
    ie: false,
    nodejs: "16.6.0",
    opera: "78",
    opera_android: "65",
    safari: "15.4",
    safari_ios: "15.4",
    samsunginternet_android: "16.0",
    webview_android: "92",
  },
  ArrayPrototypeIncludes: {
    chrome: "47",
    chrome_android: "47",
    deno: "1.0",
    edge: "14",
    firefox: "43",
    firefox_android: "43",
    ie: false,
    nodejs: "6.0.0",
    opera: "34",
    opera_android: "34",
    safari: "9",
    safari_ios: "9",
    samsunginternet_android: "5.0",
    webview_android: "47",
  },
  ArraySpread: {
    chrome: "46",
    chrome_android: "46",
    deno: "1.0",
    edge: "12",
    firefox: "16",
    firefox_android: "16",
    ie: false,
    nodejs: "5.0.0",
    opera: "37",
    opera_android: "37",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "5.0",
    webview_android: "46",
  },
  ArrayToReversed: {
    chrome: "110",
    chrome_android: "110",
    deno: "1.31",
    edge: "110",
    firefox: "115",
    firefox_android: "115",
    ie: false,
    nodejs: "20.0.0",
    opera: "96",
    opera_android: "74",
    safari: "16",
    safari_ios: "16",
    samsunginternet_android: "21.0",
    webview_android: "110",
  },
  ArrayToSorted: {
    chrome: "110",
    chrome_android: "110",
    deno: "1.31",
    edge: "110",
    firefox: "115",
    firefox_android: "115",
    ie: false,
    nodejs: "20.0.0",
    opera: "96",
    opera_android: "74",
    safari: "16",
    safari_ios: "16",
    samsunginternet_android: "21.0",
    webview_android: "110",
  },
  ArrayToSpliced: {
    chrome: "110",
    chrome_android: "110",
    deno: "1.31",
    edge: "110",
    firefox: "115",
    firefox_android: "115",
    ie: false,
    nodejs: "20.0.0",
    opera: "96",
    opera_android: "74",
    safari: "16",
    safari_ios: "16",
    samsunginternet_android: "21.0",
    webview_android: "110",
  },
  ArrayWith: {
    chrome: "110",
    chrome_android: "110",
    deno: "1.31",
    edge: "110",
    firefox: "115",
    firefox_android: "115",
    ie: false,
    nodejs: "20.0.0",
    opera: "96",
    opera_android: "74",
    safari: "16",
    safari_ios: "16",
    samsunginternet_android: "21.0",
    webview_android: "110",
  },
  ArrowFunctions: {
    chrome: "45",
    chrome_android: "45",
    deno: "1.0",
    edge: "12",
    firefox: "22",
    firefox_android: "22",
    ie: false,
    nodejs: "4.0.0",
    opera: "32",
    opera_android: "32",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "5.0",
    webview_android: "45",
  },
  AsyncAwait: {
    chrome: "55",
    chrome_android: "55",
    deno: "1.0",
    edge: "15",
    firefox: "52",
    firefox_android: "52",
    ie: false,
    nodejs: "7.6.0",
    opera: "42",
    opera_android: "42",
    safari: "10.1",
    safari_ios: "10.3",
    samsunginternet_android: "6.0",
    webview_android: "55",
  },
  AsyncIteration: {
    chrome: "63",
    chrome_android: "63",
    deno: "1.0",
    edge: "79",
    firefox: "57",
    firefox_android: "57",
    ie: false,
    nodejs: "10.0.0",
    opera: "50",
    opera_android: "46",
    safari: "12",
    safari_ios: "12",
    samsunginternet_android: "8.0",
    webview_android: "63",
  },
  Atomics: {
    chrome: "68",
    chrome_android: "89",
    deno: "1.0",
    edge: "79",
    firefox: "78",
    firefox_android: "79",
    ie: false,
    nodejs: "8.10.0",
    opera: "55",
    opera_android: "63",
    safari: "15.2",
    safari_ios: "15.2",
    samsunginternet_android: "15.0",
    webview_android: "89",
  },
  AtomicsWaitAsync: {
    chrome: "87",
    chrome_android: "89",
    deno: "1.4",
    edge: "87",
    firefox: "145",
    firefox_android: "145",
    ie: false,
    nodejs: "16.0.0",
    opera: "75",
    opera_android: "63",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "15.0",
    webview_android: "89",
  },
  BigInt: {
    chrome: "67",
    chrome_android: "67",
    deno: "1.0",
    edge: "79",
    firefox: "68",
    firefox_android: "68",
    ie: false,
    nodejs: "10.4.0",
    opera: "54",
    opera_android: "48",
    safari: "14",
    safari_ios: "14",
    samsunginternet_android: "9.0",
    webview_android: "67",
  },
  BigInt64Array: {
    chrome: "67",
    chrome_android: "67",
    deno: "1.0",
    edge: "79",
    firefox: "68",
    firefox_android: "68",
    ie: false,
    nodejs: "10.4.0",
    opera: "54",
    opera_android: "48",
    safari: "15",
    safari_ios: "15",
    samsunginternet_android: "9.0",
    webview_android: "67",
  },
  BigUint64Array: {
    chrome: "67",
    chrome_android: "67",
    deno: "1.0",
    edge: "79",
    firefox: "68",
    firefox_android: "68",
    ie: false,
    nodejs: "10.4.0",
    opera: "54",
    opera_android: "48",
    safari: "15",
    safari_ios: "15",
    samsunginternet_android: "9.0",
    webview_android: "67",
  },
  Boolean: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  ClassStaticBlocks: {
    chrome: "94",
    chrome_android: "94",
    deno: "1.14",
    edge: "94",
    firefox: "93",
    firefox_android: "93",
    ie: false,
    nodejs: "16.11.0",
    opera: "80",
    opera_android: "66",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "17.0",
    webview_android: "94",
  },
  DataView: {
    chrome: "9",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "15",
    firefox_android: "15",
    ie: "10",
    nodejs: "0.10.0",
    opera: "12.1",
    opera_android: "12.1",
    safari: "5.1",
    safari_ios: "5",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  DataViewGetFloat16: {
    chrome: "135",
    chrome_android: "135",
    deno: "1.43",
    edge: "135",
    firefox: "129",
    firefox_android: "129",
    ie: false,
    nodejs: "24.0.0",
    opera: "120",
    opera_android: "89",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: "29.0",
    webview_android: "135",
  },
  DataViewSetFloat16: {
    chrome: "135",
    chrome_android: "135",
    deno: "1.43",
    edge: "135",
    firefox: "129",
    firefox_android: "129",
    ie: false,
    nodejs: "24.0.0",
    opera: "120",
    opera_android: "89",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: "29.0",
    webview_android: "135",
  },
  Date: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  DefaultParams: {
    chrome: "49",
    chrome_android: "49",
    deno: "1.0",
    edge: "14",
    firefox: "15",
    firefox_android: "15",
    ie: false,
    nodejs: "6.0.0",
    opera: "36",
    opera_android: "36",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "5.0",
    webview_android: "49",
  },
  Destructuring: {
    chrome: "49",
    chrome_android: "49",
    deno: "1.0",
    edge: "14",
    firefox: "41",
    firefox_android: "41",
    ie: false,
    nodejs: "6.0.0",
    opera: "36",
    opera_android: "36",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "5.0",
    webview_android: "49",
  },
  DuplicateNamedCaptureGroups: {
    chrome: "125",
    chrome_android: "125",
    deno: "1.44",
    edge: "125",
    firefox: "129",
    firefox_android: "129",
    ie: false,
    nodejs: "23.0.0",
    opera: "111",
    opera_android: "83",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "27.0",
    webview_android: "125",
  },
  DynamicImport: {
    chrome: "63",
    chrome_android: "63",
    deno: "1.0",
    edge: "79",
    firefox: "67",
    firefox_android: "67",
    ie: false,
    nodejs: "12.17.0",
    opera: "50",
    opera_android: "46",
    safari: "11.1",
    safari_ios: "11.3",
    samsunginternet_android: "8.0",
    webview_android: "63",
  },
  ErgonomicBrandChecks: {
    chrome: "91",
    chrome_android: "91",
    deno: "1.9",
    edge: "91",
    firefox: "90",
    firefox_android: "90",
    ie: false,
    nodejs: "16.4.0",
    opera: "77",
    opera_android: "64",
    safari: "15",
    safari_ios: "15",
    samsunginternet_android: "16.0",
    webview_android: "91",
  },
  Error: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "6",
    nodejs: "0.10.0",
    opera: "4",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  ErrorCause: {
    chrome: "93",
    chrome_android: "93",
    deno: "1.13",
    edge: "93",
    firefox: "91",
    firefox_android: "91",
    ie: false,
    nodejs: "16.9.0",
    opera: "79",
    opera_android: "66",
    safari: "15",
    safari_ios: "15",
    samsunginternet_android: "17.0",
    webview_android: "93",
  },
  ErrorIsError: {
    chrome: "134",
    chrome_android: "134",
    deno: "2.2",
    edge: "134",
    firefox: "138",
    firefox_android: "138",
    ie: false,
    nodejs: "24.0.0",
    opera: "119",
    opera_android: "88",
    safari: "18.4",
    safari_ios: "18.4",
    samsunginternet_android: "29.0",
    webview_android: "134",
  },
  EvalError: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "5",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  ExponentOperator: {
    chrome: "52",
    chrome_android: "52",
    deno: "1.0",
    edge: "14",
    firefox: "52",
    firefox_android: "52",
    ie: false,
    nodejs: "7.0.0",
    opera: "39",
    opera_android: "41",
    safari: "10.1",
    safari_ios: "10.3",
    samsunginternet_android: "6.0",
    webview_android: "51",
  },
  FinalizationRegistry: {
    chrome: "84",
    chrome_android: "84",
    deno: "1.0",
    edge: "84",
    firefox: "79",
    firefox_android: "79",
    ie: false,
    nodejs: "14.6.0",
    opera: "70",
    opera_android: "60",
    safari: "14.1",
    safari_ios: "14.5",
    samsunginternet_android: "14.0",
    webview_android: "84",
  },
  Float16Array: {
    chrome: "135",
    chrome_android: "135",
    deno: "1.43",
    edge: "135",
    firefox: "129",
    firefox_android: "129",
    ie: false,
    nodejs: "24.0.0",
    opera: "120",
    opera_android: "89",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: "29.0",
    webview_android: "135",
  },
  Float32Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  Float64Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  ForOf: {
    chrome: "38",
    chrome_android: "38",
    deno: "1.0",
    edge: "12",
    firefox: "13",
    firefox_android: "14",
    ie: false,
    nodejs: "0.12.0",
    opera: "25",
    opera_android: "25",
    safari: "7",
    safari_ios: "7",
    samsunginternet_android: "3.0",
    webview_android: "38",
  },
  Function: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "4",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  Hashbang: {
    chrome: "74",
    chrome_android: "74",
    deno: "1.0",
    edge: "79",
    firefox: "67",
    firefox_android: "67",
    ie: false,
    nodejs: "0.10.0",
    opera: "62",
    opera_android: "53",
    safari: "13.1",
    safari_ios: "13.4",
    samsunginternet_android: "11.0",
    webview_android: "74",
  },
  Infinity: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "4",
    nodejs: "0.10.0",
    opera: "4",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  Int16Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  Int32Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  Int8Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  Intl: {
    chrome: "24",
    chrome_android: "25",
    deno: "1.8",
    edge: "12",
    firefox: "29",
    firefox_android: "56",
    ie: "11",
    nodejs: "0.12.0",
    opera: "15",
    opera_android: "14",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "1.5",
    webview_android: "4.4",
  },
  IntlDurationFormat: {
    chrome: "129",
    chrome_android: "129",
    deno: "1.46",
    edge: "129",
    firefox: "136",
    firefox_android: "136",
    ie: false,
    nodejs: "23.0.0",
    opera: "115",
    opera_android: "86",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "28.0",
    webview_android: "129",
  },
  Iterator: {
    chrome: "38",
    chrome_android: "38",
    deno: "1.0",
    edge: "12",
    firefox: "17",
    firefox_android: "17",
    ie: false,
    nodejs: "0.12.0",
    opera: "25",
    opera_android: "25",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "3.0",
    webview_android: "38",
  },
  IteratorConcat: {
    chrome: "146",
    chrome_android: "146",
    deno: "2.7.2",
    edge: "146",
    firefox: "147",
    firefox_android: "147",
    ie: false,
    nodejs: "26.0.0",
    opera: "130",
    opera_android: "97",
    safari: "26.4",
    safari_ios: "26.4",
    samsunginternet_android: false,
    webview_android: "146",
  },
  JSON: {
    chrome: "3",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "3.5",
    firefox_android: "4",
    ie: "8",
    nodejs: "0.10.0",
    opera: "10.5",
    opera_android: "11",
    safari: "4",
    safari_ios: "4",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  JSONIsRawJSON: {
    chrome: "114",
    chrome_android: "114",
    deno: "1.33",
    edge: "114",
    firefox: "135",
    firefox_android: "135",
    ie: false,
    nodejs: "21.0.0",
    opera: "100",
    opera_android: "76",
    safari: "18.4",
    safari_ios: "18.4",
    samsunginternet_android: "23.0",
    webview_android: "114",
  },
  JSONRawJSON: {
    chrome: "114",
    chrome_android: "114",
    deno: "1.33",
    edge: "114",
    firefox: "135",
    firefox_android: "135",
    ie: false,
    nodejs: "21.0.0",
    opera: "100",
    opera_android: "76",
    safari: "18.4",
    safari_ios: "18.4",
    samsunginternet_android: "23.0",
    webview_android: "114",
  },
  LogicalAssignment: {
    chrome: "85",
    chrome_android: "85",
    deno: "1.2",
    edge: "85",
    firefox: "79",
    firefox_android: "79",
    ie: false,
    nodejs: "15.0.0",
    opera: "71",
    opera_android: "60",
    safari: "14",
    safari_ios: "14",
    samsunginternet_android: "14.0",
    webview_android: "85",
  },
  Map: {
    chrome: "38",
    chrome_android: "38",
    deno: "1.0",
    edge: "12",
    firefox: "13",
    firefox_android: "14",
    ie: "11",
    nodejs: "0.12.0",
    opera: "25",
    opera_android: "25",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "3.0",
    webview_android: "38",
  },
  MapGetOrInsert: {
    chrome: "145",
    chrome_android: "145",
    deno: "2.6.7",
    edge: "145",
    firefox: "144",
    firefox_android: "144",
    ie: false,
    nodejs: "26.0.0",
    opera: "129",
    opera_android: "96",
    safari: "26.2",
    safari_ios: "26.2",
    samsunginternet_android: false,
    webview_android: "145",
  },
  MapGetOrInsertComputed: {
    chrome: "145",
    chrome_android: "145",
    deno: "2.6.7",
    edge: "145",
    firefox: "144",
    firefox_android: "144",
    ie: false,
    nodejs: "26.0.0",
    opera: "129",
    opera_android: "96",
    safari: "26.2",
    safari_ios: "26.2",
    samsunginternet_android: false,
    webview_android: "145",
  },
  MapGroupBy: {
    chrome: "117",
    chrome_android: "117",
    deno: "1.37",
    edge: "117",
    firefox: "119",
    firefox_android: "119",
    ie: false,
    nodejs: "21.0.0",
    opera: "103",
    opera_android: "78",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "24.0",
    webview_android: "117",
  },
  Math: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  MathF16Round: {
    chrome: "135",
    chrome_android: "135",
    deno: "1.43",
    edge: "135",
    firefox: "129",
    firefox_android: "129",
    ie: false,
    nodejs: "24.0.0",
    opera: "120",
    opera_android: "89",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: "29.0",
    webview_android: "135",
  },
  MathSumPrecise: {
    chrome: "147",
    chrome_android: "147",
    deno: "2.7.8",
    edge: "147",
    firefox: "137",
    firefox_android: "137",
    ie: false,
    nodejs: false,
    opera: "131",
    opera_android: "98",
    safari: "26.2",
    safari_ios: "26.2",
    samsunginternet_android: false,
    webview_android: "147",
  },
  NaN: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "4",
    nodejs: "0.10.0",
    opera: "4",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  NullishCoalescing: {
    chrome: "80",
    chrome_android: "80",
    deno: "1.0",
    edge: "80",
    firefox: "72",
    firefox_android: "79",
    ie: false,
    nodejs: "14.0.0",
    opera: "67",
    opera_android: "57",
    safari: "13.1",
    safari_ios: "13.4",
    samsunginternet_android: "13.0",
    webview_android: "80",
  },
  Number: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  NumericSeparators: {
    chrome: "75",
    chrome_android: "75",
    deno: "1.2",
    edge: "79",
    firefox: "70",
    firefox_android: "79",
    ie: false,
    nodejs: "12.5.0",
    opera: "62",
    opera_android: "54",
    safari: "13",
    safari_ios: "13",
    samsunginternet_android: "11.0",
    webview_android: "75",
  },
  Object: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "1",
  },
  ObjectEntries: {
    chrome: "54",
    chrome_android: "54",
    deno: "1.0",
    edge: "14",
    firefox: "47",
    firefox_android: "47",
    ie: false,
    nodejs: "7.0.0",
    opera: "41",
    opera_android: "41",
    safari: "10.1",
    safari_ios: "10.3",
    samsunginternet_android: "6.0",
    webview_android: "54",
  },
  ObjectFromEntries: {
    chrome: "73",
    chrome_android: "73",
    deno: "1.0",
    edge: "79",
    firefox: "63",
    firefox_android: "63",
    ie: false,
    nodejs: "12.0.0",
    opera: "60",
    opera_android: "52",
    safari: "12.1",
    safari_ios: "12.2",
    samsunginternet_android: "11.0",
    webview_android: "73",
  },
  ObjectGroupBy: {
    chrome: "117",
    chrome_android: "117",
    deno: "1.37",
    edge: "117",
    firefox: "119",
    firefox_android: "119",
    ie: false,
    nodejs: "21.0.0",
    opera: "103",
    opera_android: "78",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "24.0",
    webview_android: "117",
  },
  ObjectHasOwn: {
    chrome: "93",
    chrome_android: "93",
    deno: "1.13",
    edge: "93",
    firefox: "92",
    firefox_android: "92",
    ie: false,
    nodejs: "16.9.0",
    opera: "79",
    opera_android: "66",
    safari: "15.4",
    safari_ios: "15.4",
    samsunginternet_android: "17.0",
    webview_android: "93",
  },
  ObjectSpread: {
    chrome: "60",
    chrome_android: "60",
    deno: "1.0",
    edge: "79",
    firefox: "55",
    firefox_android: "55",
    ie: false,
    nodejs: "8.3.0",
    opera: "47",
    opera_android: "44",
    safari: "11.1",
    safari_ios: "11.3",
    samsunginternet_android: "8.2",
    webview_android: "60",
  },
  ObjectValues: {
    chrome: "54",
    chrome_android: "54",
    deno: "1.0",
    edge: "14",
    firefox: "47",
    firefox_android: "47",
    ie: false,
    nodejs: "7.0.0",
    opera: "41",
    opera_android: "41",
    safari: "10.1",
    safari_ios: "10.3",
    samsunginternet_android: "6.0",
    webview_android: "54",
  },
  OptionalCatchBinding: {
    chrome: "66",
    chrome_android: "66",
    deno: "1.0",
    edge: "79",
    firefox: "58",
    firefox_android: "58",
    ie: false,
    nodejs: "10.0.0",
    opera: "53",
    opera_android: "47",
    safari: "11.1",
    safari_ios: "11.3",
    samsunginternet_android: "9.0",
    webview_android: "66",
  },
  OptionalChaining: {
    chrome: "80",
    chrome_android: "80",
    deno: "1.0",
    edge: "80",
    firefox: "74",
    firefox_android: "79",
    ie: false,
    nodejs: "14.0.0",
    opera: "67",
    opera_android: "57",
    safari: "13.1",
    safari_ios: "13.4",
    samsunginternet_android: "13.0",
    webview_android: "80",
  },
  PrivateClassFields: {
    chrome: "74",
    chrome_android: "74",
    deno: "1.0",
    edge: "79",
    firefox: "90",
    firefox_android: "90",
    ie: false,
    nodejs: "12.0.0",
    opera: "62",
    opera_android: "53",
    safari: "14.1",
    safari_ios: "14.5",
    samsunginternet_android: "11.0",
    webview_android: "74",
  },
  Promise: {
    chrome: "32",
    chrome_android: "32",
    deno: "1.0",
    edge: "12",
    firefox: "29",
    firefox_android: "29",
    ie: false,
    nodejs: "0.12.0",
    opera: "19",
    opera_android: "19",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "2.0",
    webview_android: "4.4.3",
  },
  PromiseAllSettled: {
    chrome: "76",
    chrome_android: "76",
    deno: "1.0",
    edge: "79",
    firefox: "71",
    firefox_android: "79",
    ie: false,
    nodejs: "12.9.0",
    opera: "63",
    opera_android: "54",
    safari: "13",
    safari_ios: "13",
    samsunginternet_android: "12.0",
    webview_android: "76",
  },
  PromiseAny: {
    chrome: "85",
    chrome_android: "85",
    deno: "1.2",
    edge: "85",
    firefox: "79",
    firefox_android: "79",
    ie: false,
    nodejs: "15.0.0",
    opera: "71",
    opera_android: "60",
    safari: "14",
    safari_ios: "14",
    samsunginternet_android: "14.0",
    webview_android: "85",
  },
  PromiseFinally: {
    chrome: "63",
    chrome_android: "63",
    deno: "1.0",
    edge: "18",
    firefox: "58",
    firefox_android: "58",
    ie: false,
    nodejs: "10.0.0",
    opera: "50",
    opera_android: "46",
    safari: "11.1",
    safari_ios: "11.3",
    samsunginternet_android: "8.0",
    webview_android: "63",
  },
  PromiseReject: {
    chrome: "32",
    chrome_android: "32",
    deno: "1.0",
    edge: "12",
    firefox: "29",
    firefox_android: "29",
    ie: false,
    nodejs: "0.12.0",
    opera: "19",
    opera_android: "19",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "2.0",
    webview_android: "4.4.3",
  },
  PromiseResolve: {
    chrome: "32",
    chrome_android: "32",
    deno: "1.0",
    edge: "12",
    firefox: "29",
    firefox_android: "29",
    ie: false,
    nodejs: "0.12.0",
    opera: "19",
    opera_android: "19",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "2.0",
    webview_android: "4.4.3",
  },
  PromiseTry: {
    chrome: "128",
    chrome_android: "128",
    deno: "1.46",
    edge: "128",
    firefox: "134",
    firefox_android: "134",
    ie: false,
    nodejs: "23.0.0",
    opera: "114",
    opera_android: "85",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: "28.0",
    webview_android: "128",
  },
  PromiseWithResolvers: {
    chrome: "119",
    chrome_android: "119",
    deno: "1.38",
    edge: "119",
    firefox: "121",
    firefox_android: "121",
    ie: false,
    nodejs: "22.0.0",
    opera: "105",
    opera_android: "79",
    safari: "17.4",
    safari_ios: "17.4",
    samsunginternet_android: "25.0",
    webview_android: "119",
  },
  Proxy: {
    chrome: "49",
    chrome_android: "49",
    deno: "1.0",
    edge: "12",
    firefox: "18",
    firefox_android: "18",
    ie: false,
    nodejs: "6.0.0",
    opera: "36",
    opera_android: "36",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "5.0",
    webview_android: "49",
  },
  RangeError: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "5",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  ReferenceError: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "5",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  Reflect: {
    chrome: "49",
    chrome_android: "49",
    deno: "1.0",
    edge: "12",
    firefox: "42",
    firefox_android: "42",
    ie: false,
    nodejs: "6.0.0",
    opera: "36",
    opera_android: "36",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "5.0",
    webview_android: "49",
  },
  RegExp: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "4",
    nodejs: "0.10.0",
    opera: "5",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  RegExpEscape: {
    chrome: "136",
    chrome_android: "136",
    deno: "2.3.2",
    edge: "136",
    firefox: "134",
    firefox_android: "134",
    ie: false,
    nodejs: "24.0.0",
    opera: "121",
    opera_android: "90",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: "29.0",
    webview_android: "136",
  },
  RegExpUnicodeSetFlag: {
    chrome: "112",
    chrome_android: "112",
    deno: "1.32",
    edge: "112",
    firefox: "116",
    firefox_android: "116",
    ie: false,
    nodejs: "20.0.0",
    opera: "98",
    opera_android: "75",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "23.0",
    webview_android: "112",
  },
  RestSpread: {
    chrome: "47",
    chrome_android: "47",
    deno: "1.0",
    edge: "12",
    firefox: "15",
    firefox_android: "15",
    ie: false,
    nodejs: "6.0.0",
    opera: "34",
    opera_android: "34",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "5.0",
    webview_android: "47",
  },
  Set: {
    chrome: "38",
    chrome_android: "38",
    deno: "1.0",
    edge: "12",
    firefox: "13",
    firefox_android: "14",
    ie: "11",
    nodejs: "0.12.0",
    opera: "25",
    opera_android: "25",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "3.0",
    webview_android: "38",
  },
  SetDifference: {
    chrome: "122",
    chrome_android: "122",
    deno: "1.42",
    edge: "122",
    firefox: "127",
    firefox_android: "127",
    ie: false,
    nodejs: "22.0.0",
    opera: "108",
    opera_android: "81",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "26.0",
    webview_android: "122",
  },
  SetIntersection: {
    chrome: "122",
    chrome_android: "122",
    deno: "1.42",
    edge: "122",
    firefox: "127",
    firefox_android: "127",
    ie: false,
    nodejs: "22.0.0",
    opera: "108",
    opera_android: "81",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "26.0",
    webview_android: "122",
  },
  SetIsDisjointFrom: {
    chrome: "122",
    chrome_android: "122",
    deno: "1.42",
    edge: "122",
    firefox: "127",
    firefox_android: "127",
    ie: false,
    nodejs: "22.0.0",
    opera: "108",
    opera_android: "81",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "26.0",
    webview_android: "122",
  },
  SetIsSubsetOf: {
    chrome: "122",
    chrome_android: "122",
    deno: "1.42",
    edge: "122",
    firefox: "127",
    firefox_android: "127",
    ie: false,
    nodejs: "22.0.0",
    opera: "108",
    opera_android: "81",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "26.0",
    webview_android: "122",
  },
  SetIsSupersetOf: {
    chrome: "122",
    chrome_android: "122",
    deno: "1.42",
    edge: "122",
    firefox: "127",
    firefox_android: "127",
    ie: false,
    nodejs: "22.0.0",
    opera: "108",
    opera_android: "81",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "26.0",
    webview_android: "122",
  },
  SetSymmetricDifference: {
    chrome: "122",
    chrome_android: "122",
    deno: "1.42",
    edge: "122",
    firefox: "127",
    firefox_android: "127",
    ie: false,
    nodejs: "22.0.0",
    opera: "108",
    opera_android: "81",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "26.0",
    webview_android: "122",
  },
  SetUnion: {
    chrome: "122",
    chrome_android: "122",
    deno: "1.42",
    edge: "122",
    firefox: "127",
    firefox_android: "127",
    ie: false,
    nodejs: "22.0.0",
    opera: "108",
    opera_android: "81",
    safari: "17",
    safari_ios: "17",
    samsunginternet_android: "26.0",
    webview_android: "122",
  },
  SharedArrayBuffer: {
    chrome: "68",
    chrome_android: "89",
    deno: "1.0",
    edge: "79",
    firefox: "79",
    firefox_android: "79",
    ie: false,
    nodejs: "8.10.0",
    opera: "55",
    opera_android: "63",
    safari: "15.2",
    safari_ios: "15.2",
    samsunginternet_android: "15.0",
    webview_android: false,
  },
  String: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "1",
  },
  StringIsWellFormed: {
    chrome: "111",
    chrome_android: "111",
    deno: "1.32",
    edge: "111",
    firefox: "119",
    firefox_android: "119",
    ie: false,
    nodejs: "20.0.0",
    opera: "97",
    opera_android: "75",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "22.0",
    webview_android: "111",
  },
  StringMatchAll: {
    chrome: "73",
    chrome_android: "73",
    deno: "1.0",
    edge: "79",
    firefox: "67",
    firefox_android: "67",
    ie: false,
    nodejs: "12.0.0",
    opera: "60",
    opera_android: "52",
    safari: "13",
    safari_ios: "13",
    samsunginternet_android: "11.0",
    webview_android: "73",
  },
  StringPadEnd: {
    chrome: "57",
    chrome_android: "57",
    deno: "1.0",
    edge: "15",
    firefox: "48",
    firefox_android: "48",
    ie: false,
    nodejs: "8.0.0",
    opera: "44",
    opera_android: "43",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "7.0",
    webview_android: "57",
  },
  StringPadStart: {
    chrome: "57",
    chrome_android: "57",
    deno: "1.0",
    edge: "15",
    firefox: "48",
    firefox_android: "48",
    ie: false,
    nodejs: "8.0.0",
    opera: "44",
    opera_android: "43",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "7.0",
    webview_android: "57",
  },
  StringReplaceAll: {
    chrome: "85",
    chrome_android: "85",
    deno: "1.2",
    edge: "85",
    firefox: "77",
    firefox_android: "79",
    ie: false,
    nodejs: "15.0.0",
    opera: "71",
    opera_android: "60",
    safari: "13.1",
    safari_ios: "13.4",
    samsunginternet_android: "14.0",
    webview_android: "85",
  },
  StringToWellFormed: {
    chrome: "111",
    chrome_android: "111",
    deno: "1.32",
    edge: "111",
    firefox: "119",
    firefox_android: "119",
    ie: false,
    nodejs: "20.0.0",
    opera: "97",
    opera_android: "75",
    safari: "16.4",
    safari_ios: "16.4",
    samsunginternet_android: "22.0",
    webview_android: "111",
  },
  Symbol: {
    chrome: "38",
    chrome_android: "38",
    deno: "1.0",
    edge: "12",
    firefox: "36",
    firefox_android: "36",
    ie: false,
    nodejs: "0.12.0",
    opera: "25",
    opera_android: "25",
    safari: "9",
    safari_ios: "9",
    samsunginternet_android: "3.0",
    webview_android: "38",
  },
  SyntaxError: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "5",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  TemplateLiterals: {
    chrome: "41",
    chrome_android: "41",
    deno: "1.0",
    edge: "12",
    firefox: "34",
    firefox_android: "34",
    ie: false,
    nodejs: "4.0.0",
    opera: "28",
    opera_android: "28",
    safari: "9",
    safari_ios: "9",
    samsunginternet_android: "4.0",
    webview_android: "41",
  },
  TopLevelAwait: {
    chrome: "89",
    chrome_android: "89",
    deno: "1.0",
    edge: "89",
    firefox: "89",
    firefox_android: "89",
    ie: false,
    nodejs: "14.8.0",
    opera: "75",
    opera_android: "63",
    safari: "15",
    safari_ios: "15",
    samsunginternet_android: "15.0",
    webview_android: "89",
  },
  TypeError: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "5",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  URIError: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "5",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  Uint16Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  Uint32Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  Uint8Array: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  Uint8ArrayFromBase64: {
    chrome: "140",
    chrome_android: "140",
    deno: "2.5.0",
    edge: "140",
    firefox: "133",
    firefox_android: "133",
    ie: false,
    nodejs: "25.0.0",
    opera: "124",
    opera_android: "92",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: false,
    webview_android: "140",
  },
  Uint8ArrayFromHex: {
    chrome: "140",
    chrome_android: "140",
    deno: "2.5.0",
    edge: "140",
    firefox: "133",
    firefox_android: "133",
    ie: false,
    nodejs: "25.0.0",
    opera: "124",
    opera_android: "92",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: false,
    webview_android: "140",
  },
  Uint8ArraySetFromBase64: {
    chrome: "140",
    chrome_android: "140",
    deno: "2.5.0",
    edge: "140",
    firefox: "133",
    firefox_android: "133",
    ie: false,
    nodejs: "25.0.0",
    opera: "124",
    opera_android: "92",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: false,
    webview_android: "140",
  },
  Uint8ArraySetFromHex: {
    chrome: "140",
    chrome_android: "140",
    deno: "2.5.0",
    edge: "140",
    firefox: "133",
    firefox_android: "133",
    ie: false,
    nodejs: "25.0.0",
    opera: "124",
    opera_android: "92",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: false,
    webview_android: "140",
  },
  Uint8ArrayToBase64: {
    chrome: "140",
    chrome_android: "140",
    deno: "2.5.0",
    edge: "140",
    firefox: "133",
    firefox_android: "133",
    ie: false,
    nodejs: "25.0.0",
    opera: "124",
    opera_android: "92",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: false,
    webview_android: "140",
  },
  Uint8ArrayToHex: {
    chrome: "140",
    chrome_android: "140",
    deno: "2.5.0",
    edge: "140",
    firefox: "133",
    firefox_android: "133",
    ie: false,
    nodejs: "25.0.0",
    opera: "124",
    opera_android: "92",
    safari: "18.2",
    safari_ios: "18.2",
    samsunginternet_android: false,
    webview_android: "140",
  },
  Uint8ClampedArray: {
    chrome: "7",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "4",
    firefox_android: "4",
    ie: "10",
    nodejs: "0.10.0",
    opera: "11.6",
    opera_android: "12",
    safari: "5.1",
    safari_ios: "4.2",
    samsunginternet_android: "1.0",
    webview_android: "4",
  },
  WeakMap: {
    chrome: "36",
    chrome_android: "36",
    deno: "1.0",
    edge: "12",
    firefox: "6",
    firefox_android: "6",
    ie: "11",
    nodejs: "0.12.0",
    opera: "23",
    opera_android: "24",
    safari: "8",
    safari_ios: "8",
    samsunginternet_android: "3.0",
    webview_android: "37",
  },
  WeakRef: {
    chrome: "84",
    chrome_android: "84",
    deno: "1.0",
    edge: "84",
    firefox: "79",
    firefox_android: "79",
    ie: false,
    nodejs: "14.6.0",
    opera: "70",
    opera_android: "60",
    safari: "14.1",
    safari_ios: "14.5",
    samsunginternet_android: "14.0",
    webview_android: "84",
  },
  WeakSet: {
    chrome: "36",
    chrome_android: "36",
    deno: "1.0",
    edge: "12",
    firefox: "34",
    firefox_android: "34",
    ie: false,
    nodejs: "0.12.0",
    opera: "23",
    opera_android: "24",
    safari: "9",
    safari_ios: "9",
    samsunginternet_android: "3.0",
    webview_android: "37",
  },
  class: {
    chrome: "42",
    chrome_android: "42",
    deno: "1.0",
    edge: "13",
    firefox: "45",
    firefox_android: "45",
    ie: false,
    nodejs: "6.0.0",
    opera: "29",
    opera_android: "29",
    safari: "10.1",
    safari_ios: "10.3",
    samsunginternet_android: "4.0",
    webview_android: "42",
  },
  const: {
    chrome: "21",
    chrome_android: "25",
    deno: "1.0",
    edge: "12",
    firefox: "36",
    firefox_android: "36",
    ie: "11",
    nodejs: "6.0.0",
    opera: "9",
    opera_android: "10.1",
    safari: "5.1",
    safari_ios: "5",
    samsunginternet_android: "1.5",
    webview_android: "4.4",
  },
  decodeURI: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "7",
    opera_android: "10.1",
    safari: "1.1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  decodeURIComponent: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "7",
    opera_android: "10.1",
    safari: "1.1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  encodeURI: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "7",
    opera_android: "10.1",
    safari: "1.1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  encodeURIComponent: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "7",
    opera_android: "10.1",
    safari: "1.1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  escape: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  eval: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  export: {
    chrome: "61",
    chrome_android: "61",
    deno: "1.0",
    edge: "16",
    firefox: "60",
    firefox_android: "60",
    ie: false,
    nodejs: "13.2.0",
    opera: "48",
    opera_android: "45",
    safari: "10.1",
    safari_ios: "10.3",
    samsunginternet_android: "8.0",
    webview_android: "61",
  },
  extends: {
    chrome: "42",
    chrome_android: "42",
    deno: "1.0",
    edge: "13",
    firefox: "45",
    firefox_android: "45",
    ie: false,
    nodejs: "6.0.0",
    opera: "29",
    opera_android: "29",
    safari: "9",
    safari_ios: "9",
    samsunginternet_android: "4.0",
    webview_android: "42",
  },
  globalThis: {
    chrome: "71",
    chrome_android: "71",
    deno: "1.0",
    edge: "79",
    firefox: "65",
    firefox_android: "65",
    ie: false,
    nodejs: "12.0.0",
    opera: "58",
    opera_android: "50",
    safari: "12.1",
    safari_ios: "12.2",
    samsunginternet_android: "10.0",
    webview_android: "71",
  },
  import: {
    chrome: "61",
    chrome_android: "61",
    deno: "1.0",
    edge: "16",
    firefox: "60",
    firefox_android: "60",
    ie: false,
    nodejs: "12.17.0",
    opera: "48",
    opera_android: "45",
    safari: "10.1",
    safari_ios: "10.3",
    samsunginternet_android: "8.0",
    webview_android: "61",
  },
  isFinite: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "4",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  isNaN: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  let: {
    chrome: "41",
    chrome_android: "41",
    deno: "1.0",
    edge: "12",
    firefox: "44",
    firefox_android: "44",
    ie: "11",
    nodejs: "6.0.0",
    opera: "17",
    opera_android: "18",
    safari: "10",
    safari_ios: "10",
    samsunginternet_android: "4.0",
    webview_android: "41",
  },
  parseFloat: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  parseInt: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  undefined: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "5.5",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
  unescape: {
    chrome: "1",
    chrome_android: "18",
    deno: "1.0",
    edge: "12",
    firefox: "1",
    firefox_android: "4",
    ie: "3",
    nodejs: "0.10.0",
    opera: "3",
    opera_android: "10.1",
    safari: "1",
    safari_ios: "1",
    samsunginternet_android: "1.0",
    webview_android: "4.4",
  },
};

module.exports = { FEATURE_BROWSER_SUPPORT };
