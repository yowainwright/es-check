function hasOptionsCause(node) {
  ...
  return optionsArg.properties?.some(
    (prop) =>
      prop.type === "Property" && prop.key?.type === "Identifier" && prop.key?.name === "cause",
  );
}
...
function isSetConstructorCall(calleeObject) {
  // new Set() or new Set([...])
  if (calleeObject.type === "NewExpression" && calleeObject.callee?.name === "Set") {
    return true;
  }
  return false;
}
...
function isSetLikeIdentifier(calleeObject) {
  ...
  return SET_LIKE_NAMES.has(name) || name.startsWith("set") || name.endsWith("set");
}
