...
function checkMap(node, astInfo) {
  ...
  if (astInfo.object) {
    if (node.callee && node.callee.object && node.callee.object.name === astInfo.object) {
      if (astInfo.property && node.callee.property && node.callee.property.name !== astInfo.property) {
        return false;
      }
    } else if (node.callee && node.callee.type === 'Identifier' && node.callee.name === astInfo.object) {
      return true;
    } else {
      return false;
    }
  ...
  if (astInfo.property && !astInfo.object) {
    if (node.callee && node.callee.property && node.callee.property.name === astInfo.property) {
      if (astInfo.excludeObjects && node.callee.object && node.callee.object.name) {
