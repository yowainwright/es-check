...
const MAP_CONSTRUCTORS = new Set(["Map", "WeakMap"]);

const MAP_LIKE_NAMES = new Set(["map", "maps", "map1", "map2", "weakmap", "weakmaps"]);

const COMMON_NON_MAP_NAMES = new Set(["sitemap", "roadmap"]);

function isMapConstructorCall(calleeObject) {
  const constructorName = calleeObject.callee?.name;
  const isNewExpression = calleeObject.type === "NewExpression";
  return isNewExpression && MAP_CONSTRUCTORS.has(constructorName);
}

function isMapLikeIdentifier(calleeObject) {
  if (calleeObject.type !== "Identifier") return false;

  const name = calleeObject.name.toLowerCase();
  if (COMMON_NON_MAP_NAMES.has(name)) return false;

  const hasKnownMapName = MAP_LIKE_NAMES.has(name);
  const hasMapSuffix = name.endsWith("map");
  return hasKnownMapName || hasMapSuffix;
}
...
function isMapLikeCall(node) {
  ...
  return isMapConstructorCall(calleeObject) || isMapLikeIdentifier(calleeObject);
}
...
function checkMap(node, astInfo) {
  ...
  const hasMapLikeCallMismatch = astInfo.requireMapLikeCall && !isMapLikeCall(node);
  ...
  const mismatchChecks = [
    ...
    hasSetLikeCallMismatch || hasMapLikeCallMismatch,
  ];
