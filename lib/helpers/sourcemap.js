...
function parseSegment(segment, state) {
  ...
  const nextState = {
    ...
    previousSourceLine: hasOriginalLine ? state.previousSourceLine + values[2] : state.previousSourceLine,
...
function parseLine(line, state) {
  ...
  const result = segments.reduce((acc, segment) => {
    ...
    return hasMapping
      ? { mappings: [...acc.mappings, parsed.mapping], state: parsed.nextState }
      : acc;
