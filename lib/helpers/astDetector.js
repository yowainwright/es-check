...
function traverseFeatures(node, context, state) {
  ...
  visitChildNodes(node, (child) => {
    ...
    const assignmentScope = isProgram ? context.assignmentScope : null;
...
function recordTopLevelAssignment(node, context) {
  const isExpressionStatement = node.type === "ExpressionStatement";
  if (!isExpressionStatement) return;
  if (!context.assignmentScope) return;

  addAssignedGlobalName(node.expression, context.assignmentScope.names);
}
