...
function createScope(parent = null) {
  return { parent, names: new Set(), mapReceivers: new Set() };
}
...
function isKnownMapReceiver(key, context) {
  ...
  const isThisMember = key.startsWith("this.");
  return isThisMember && context.mapReceivers.has(key);
}
...
function traverseFeatures(node, context, state) {
  ...
  recordMapReceiver(node, context, state);
  ...
  if (candidates) {
    const matchContext = {
      ...
      mapReceivers: state.mapReceivers,
      parent,
      scope,
    };
...
function recordMapReceiver(node, context, state) {
  const receiverKey = getMapReceiverDeclarationKey(node, context);
  if (!receiverKey) return;

  context.scope.mapReceivers.add(receiverKey);
  if (receiverKey.startsWith("this.")) state.mapReceivers.add(receiverKey);
}
...
function detectFeaturesFromAST(ast) {
  ...
  traverseFeatures(
    ...
    { foundFeatures, mapReceivers: new Set(), remaining, remainingGlobals },
  );
