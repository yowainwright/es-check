...
const RECEIVER_EXIT_JOIN_TYPES = new Set([
  "TryStatement",
  "ForStatement",
  "ForInStatement",
  "ForOfStatement",
...
function mergeReceiverScopes(scope, branches) {
  if (!scope) return;
  // Retain a receiver only when every incoming path still knows it is a Map.
...
function traverseChildFeatures(node, context, state, newFunctionDepth) {
  ...
  if (node.type === "ReturnStatement") recordReceiverExit(context);
}

function traverseFunction(node, context, state, newFunctionDepth) {
  // Visiting a deferred body must not execute its writes in the enclosing scope.
  ...
  childContext.preserveReceiverExits = false;
...
function traverseProgram(node, context, state, newFunctionDepth) {
  ...
  traverseStatements(node.body, childContext, state);
}
...
function traverseStatements(statements, context, state) {
  ...
  visitArray(statements, (statement) => {
    traverseFeatures(statement, childContext, state);
    if (!isAbruptStatement(statement)) return;
    const scope = cloneScope(childContext.scope);
    childContext = Object.assign({}, childContext, { scope, reachable: false });
  });
}

function isAbruptStatement(node, includeContinue = true) {
  if (!node) return false;
  if (node.type === "ContinueStatement") return includeContinue;
  const abruptTypes = ["ReturnStatement", "ThrowStatement", "BreakStatement"];
  if (abruptTypes.includes(node.type)) return true;
  if (node.type === "BlockStatement") {
    return node.body.some((statement) => isAbruptStatement(statement, includeContinue));
  }
  if (node.type !== "IfStatement") return false;
  return (
    isAbruptStatement(node.consequent, includeContinue) &&
    isAbruptStatement(node.alternate, includeContinue)
  );
}

function recordReceiverExit(context) {
  if (context.reachable === false) return;
  let scope = context.scope;
  while (scope) {
    if (scope.isFunction) {
      if (scope.receiverExits) scope.receiverExits.add(cloneScope(scope));
      return;
    }
    scope = scope.parent;
  }
...
function traverseClassConstructor(node, context, state) {
  ...
  const exits = Array.from(scope.receiverExits).concat(cloneScope(scope));
...
function createChildContext(context, parent, guardedGlobals, newFunctionDepth) {
  const preserveReceiverExits =
    context.preserveReceiverExits || RECEIVER_EXIT_JOIN_TYPES.has(parent.type);
  return Object.assign({}, context, {
    functionDepth: newFunctionDepth,
    parent,
    assignmentScope: null,
    guardedGlobals,
    preserveReceiverExits,
  });
}
...
function traverseIfStatement(node, context, state, newFunctionDepth) {
  ...
  const branches = getContinuingReceiverBranches(node, context, consequent, alternate);
  mergeReceiverScopes(context.scope, branches);
}

function getContinuingReceiverBranches(node, context, consequent, alternate) {
  const branches = [consequent, alternate];
  if (context.preserveReceiverExits) return branches;
  const consequentExits = isAbruptStatement(node.consequent);
  const alternateExits = isAbruptStatement(node.alternate);
  if (consequentExits === alternateExits) return branches;
  if (consequentExits) return [alternate];
  return [consequent];
}
...
function traverseReceiverBranch(node, parent, context, state, guardNames) {
  ...
  traverseFeatures(node, childContext, state);
  return childContext.scope;
}
...
function traverseForStatement(node, context, state, depth) {
  ...
  const iteration = createReceiverBranchContext(childContext, node, guards, depth);
  traverseFeatures(node.body, iteration, state);
  const updateContext = isAbruptStatement(node.body, false)
    ? createReceiverBranchContext(iteration, node, guards, depth)
    : iteration;
  traverseFeatures(node.update, updateContext, state);
  mergeReceiverScopes(loopScope, [loopScope, iteration.scope]);
}
...
function traverseForInOfStatement(node, context, state, newFunctionDepth) {
  ...
  const iteration = createReceiverBranchContext(
    loopContext,
    node,
    context.guardedGlobals,
    newFunctionDepth,
...
function traverseWhileStatement(node, context, state, newFunctionDepth) {
  ...
  traverseFeatures(node.body, iteration, state);
  mergeReceiverScopes(context.scope, [context.scope, iteration.scope]);
}

function traverseDoWhileStatement(node, context, state, newFunctionDepth) {
  const bodyContext = createAssignmentChildContext(context, node, newFunctionDepth);
  traverseFeatures(node.body, bodyContext, state);
  const testContext = createChildContext(context, node, context.guardedGlobals, newFunctionDepth);
  traverseFeatures(node.test, testContext, state);
}
...
function traverseSwitchStatement(node, context, state, newFunctionDepth) {
  ...
  visitArray(node.cases, (switchCase, index) => {
    ...
    const isAbrupt = switchCase.consequent.some((statement) => isAbruptStatement(statement));
    const isExit = isAbrupt || index === node.cases.length - 1;
    if (isExit) branches.add(branch.scope);
    fallthrough = isAbrupt ? null : branch.scope;
  });
...
function traverseSwitchCase(node, parent, context, state, fallthrough, outerContext) {
  ...
  traverseStatements(node.consequent, branch, state);
  return branch;
}
