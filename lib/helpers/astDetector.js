...
function matchesFeature(node, astInfo, context = {}) {
  ...
  // TopLevelAwait: only match AwaitExpression if it's at the top level (not inside a function)
  if (astInfo.topLevel && node.type === "AwaitExpression") {
    return context.isTopLevel === true;
  }

  // ErgonomicBrandChecks: only match 'in' operator if left side is a private identifier
    ...
    for (const key of CHILD_KEYS) {
      ...
      if (Array.isArray(child)) {
        for (let i = child.length - 1; i >= 0; i--) {
          if (child[i]) stack.push({ node: child[i], functionDepth: newFunctionDepth });
        }
      } else if (child.type) {
