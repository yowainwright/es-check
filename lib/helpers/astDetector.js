...
function getTypeofComparison(node) {
  ...
  return normalizeTypeofComparison(rightComparison, flipRelationalOperator(node.operator));
}
...
function flipRelationalOperator(operator) {
  if (operator === "<") return ">";
  if (operator === ">") return "<";
  return operator;
}

function normalizeTypeofComparison(comparison, operator) {
  const isRelational = operator === "<" || operator === ">";
  if (!isRelational) return Object.assign({}, comparison, { operator });

  const isMinifiedUndefinedCheck = comparison.value === "u";
  if (!isMinifiedUndefinedCheck) return null;

  const isUndefinedBranch = operator === ">";
  const equalityOperator = isUndefinedBranch ? "===" : "!==";
  return Object.assign({}, comparison, { operator: equalityOperator, value: "undefined" });
}
