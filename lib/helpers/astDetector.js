...
function createProgramScope(ast) {
  ...
  collectAssignedGlobalNames(ast, scope.names);
  return scope;
}

function collectAssignedGlobalNames(node, names) {
  const isNotNode = !isNodeRecord(node);
  if (isNotNode) return;
  if (FUNCTION_SCOPE_TYPES.has(node.type)) return;

  const isAssignment = node.type === "AssignmentExpression";
  const target = isAssignment ? node.left : null;
  ...
  visitChildNodes(node, (child) => {
    collectAssignedGlobalNames(child, names);
  });
}
