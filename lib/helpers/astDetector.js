...
function createScope(parent = null) {
  return { parent, names: new Set(), mapReceivers: new Set(), nonMapReceivers: new Set() };
}
...
function cloneScope(scope) {
  return {
    parent: scope.parent,
    names: scope.names,
    mapReceivers: new Set(scope.mapReceivers),
    nonMapReceivers: new Set(scope.nonMapReceivers),
  };
}
...
function createClassBodyScope(parentScope) {
  return createScope(parentScope);
}
...
function isKnownMapReceiver(key, context) {
  ...
  while (scope) {
    if (scope.nonMapReceivers.has(key)) return false;
...
function traverseFeatures(node, context, state) {
    ...
    visitArray(node.body, (child) => {
      traverseFeatures(
        child,
        {
          ...
          guardedGlobals: context.guardedGlobals,
          receiverWrites: context.receiverWrites,
        },
        state,
      );
    });
  ...
  if (node.type === "ClassBody") {
    const classScope = createClassBodyScope(scope);
    visitArray(node.body, (child) => {
      traverseFeatures(
        child,
        {
  ...
  const isForInOrOf = node.type === "ForInStatement" || node.type === "ForOfStatement";
  if (isForInOrOf) {
    traverseForInOfStatement(node, context, state, newFunctionDepth);
    return;
  }

  if (node.type === "SwitchStatement") {
    traverseSwitchStatement(node, context, state, newFunctionDepth);
    return;
  }
...
function getMapReceiverDeclarationKey(node, context) {
  const variableKey = getVariableMapReceiverKey(node, context);
  if (variableKey) return variableKey;

  const assignmentKey = getAssignmentMapReceiverKey(node, context);
  if (assignmentKey) return assignmentKey;

  return getClassFieldMapReceiverKey(node, context);
}

function getVariableMapReceiverKey(node, context) {
  const isVariable = node.type === "VariableDeclarator";
  const hasVariableMapInit = isVariable && isMapConstructorExpression(node.init, context);
  if (!hasVariableMapInit) return null;

  const hasIdentifierId = node.id?.type === "Identifier";
  if (!hasIdentifierId) return null;
  return node.id.name;
}

function getAssignmentMapReceiverKey(node, context) {
  const isAssignment = node.type === "AssignmentExpression" && node.operator === "=";
  const hasAssignmentMapValue = isAssignment && isMapConstructorExpression(node.right, context);
  if (!hasAssignmentMapValue) return null;

  return getStaticReferenceKey(node.left);
}

function getClassFieldMapReceiverKey(node, context) {
  const isClassField = node.type === "PropertyDefinition";
  const hasStaticKey = node.key?.type === "Identifier" && !node.computed;
  const hasClassFieldMapValue = isClassField && isMapConstructorExpression(node.value, context);
  const hasStaticClassFieldMapValue = hasClassFieldMapValue && hasStaticKey;
  if (!hasStaticClassFieldMapValue) return null;

  return `this.${node.key.name}`;
}

function getAssignedMapReceiverKey(node) {
  const isAssignment = node.type === "AssignmentExpression" && node.operator === "=";
...
function getReceiverTrackingScope(key, context) {
  if (context.receiverWrites) return context.scope;

  const rootName = getReceiverRootName(key);
  if (!rootName) return context.scope;

  let scope = context.scope;
  while (scope) {
    if (scope.names.has(rootName)) return scope;
    scope = scope.parent;
  }
...
function markMapReceiver(scope, key) {
  scope.nonMapReceivers.delete(key);
  scope.mapReceivers.add(key);
}

function markNonMapReceiver(scope, key) {
  scope.mapReceivers.delete(key);
  scope.nonMapReceivers.add(key);
}

function recordMapReceiver(node, context) {
  const assignedKey = getAssignedMapReceiverKey(node);
  const receiverKey = getMapReceiverDeclarationKey(node, context) || assignedKey;
  if (!receiverKey) return;

  const trackingScope = getReceiverTrackingScope(receiverKey, context);
  const mapReceiverKey = getMapReceiverDeclarationKey(node, context);
  if (mapReceiverKey) {
    markMapReceiver(trackingScope, receiverKey);
  } else {
    markNonMapReceiver(trackingScope, receiverKey);
  }

  if (context.receiverWrites) context.receiverWrites.add(receiverKey);
}
...
function createReceiverBranchContext(context, parent, guardedGlobals, newFunctionDepth, writes) {
  return {
...
function traverseGuardedLogicalExpression(node, context, state, newFunctionDepth) {
  ...
  const receiverWrites = new Set();
  const rightContext = createReceiverBranchContext(
    context,
    node,
    guardedGlobals,
    newFunctionDepth,
  ...
  invalidateReceiverWrites(context.scope, receiverWrites);
}
...
function traverseConditionalExpression(node, context, state, newFunctionDepth) {
  ...
  const receiverWrites = new Set();
  const consequentContext = createReceiverBranchContext(
    context,
    node,
    guardedGlobals,
    newFunctionDepth,
  ...
  const alternateContext = createReceiverBranchContext(
    context,
    node,
...
function traverseForStatement(node, context, state, newFunctionDepth) {
  ...
  const initContext = createAssignmentChildContext(loopContext, node, newFunctionDepth);
...
function traverseForInOfStatement(node, context, state, newFunctionDepth) {
  ...
  const leftContext = createAssignmentChildContext(loopContext, node, newFunctionDepth);
  traverseFeatures(node.left, leftContext, state);
  ...
  traverseFeatures(node.right, childContext, state);
  traverseFeatures(node.body, childContext, state);
  invalidateReceiverWrites(context.scope, receiverWrites);
}
...
function traverseSwitchStatement(node, context, state, newFunctionDepth) {
  ...
  const switchScope = createSwitchScope(node, context.scope);
  const receiverWrites = new Set();
  visitArray(node.cases, (switchCase) => {
    const isDefaultOnlyCase = isOnlyDefaultCase(node, switchCase);
    const assignmentScope = isDefaultOnlyCase ? context.assignmentScope : null;
    traverseFeatures(
      switchCase,
      {
        scope: switchScope,
...
function invalidateReceiverWrites(scope, receiverWrites) {
  receiverWrites.forEach((receiverKey) => {
    markNonMapReceiver(scope, receiverKey);
  });
}
