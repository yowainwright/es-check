...
function collectTypeofNamesFromNode(node, names) {
  ...
  if (FUNCTION_SCOPE_TYPES.has(node.type)) return;
  if (node.type === "ConditionalExpression") {
    collectConditionalTypeofNames(node, names);
    return;
  }
  if (isDisjunctiveGuard(node)) return;
...
function collectConditionalTypeofNames(node, names) {
  if (isFalseLiteral(node.alternate)) {
    collectTypeofNamesFromNode(node.consequent, names);
    return;
  }

  if (isFalseLiteral(node.consequent)) {
    collectTypeofNamesFromNode(node.alternate, names);
  }
}
...
function isDisjunctiveGuard(node) {
  const isLogical = node.type === "LogicalExpression";
  const isOrLike = node.operator === "||" || node.operator === "??";
  const isOrLogical = isLogical && isOrLike;
  if (isOrLogical) return true;
  return false;
}
