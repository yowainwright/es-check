...
const defaultDetectionContext = {
  featureKeys,
  featuresByNodeType,
};

function createFeatureDetectionContext(features = ES_FEATURES) {
  return {
    featureKeys: Object.keys(features),
    featuresByNodeType: buildFeatureIndex(features),
  };
}
...
function detectFeaturesFromAST(
  ast,
  detectionContext = defaultDetectionContext,
) {
  const keys = detectionContext.featureKeys || featureKeys;
  const indexedFeatures =
    detectionContext.featuresByNodeType || featuresByNodeType;
  ...
  while (stack.length > 0 && remaining.size > 0) {
    ...
    const candidates = indexedFeatures[node.type];
    if (candidates) {
