...
function traverseConditionalLoop(node, context, state) {
  ...
  const continues = traverseFeatures(node.body, iteration, state) !== false;
  const paths = getLoopReceiverPaths(iteration, continues, context.loopLabels);
  const updated = traverseLoopUpdate([node.update, node.test], iteration, paths.iterations, state);
...
function traverseLoopUpdate(nodes, context, incoming, state) {
  ...
  visitArray(nodes, (node) => traverseFeatures(node, updateContext, state));
