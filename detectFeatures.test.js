...
describe('detectFeatures', () => {
  describe('Basic functionality', () => {
    it('should detect ES6 features', async () => {
      ...
      const { foundFeatures, unsupportedFeatures } = await detectFeatures(code, 6, 'script', new Set());
    ...
    it('should detect unsupported features for lower versions', async () => {
      const code = `
        const x = 1;
        let y = 2;
      `;
      ...
      try {
        await detectFeatures(code, 5, 'script', new Set());
    ...
    it('should respect the ignoreList', async () => {
      const code = `
        const x = 1;
        let y = 2;
      `;
      ...
      try {
        const { unsupportedFeatures } = await detectFeatures(code, 5, 'script', ignoreList);
  ...
  describe('detectPolyfills', () => {
    it('should detect polyfills when checkForPolyfills is enabled', async () => {
      const code = `
      ...
      try {
        const { unsupportedFeatures } = await detectFeatures(code, 2020, 'module', new Set(), options);
    ...
    it('should detect polyfill patterns in code', async () => {
      const code = `
      ...
      try {
        const { unsupportedFeatures } = await detectFeatures(code, 2020, 'script', new Set(), options);
        assert.strictEqual(unsupportedFeatures.length, 0, 'Should have no unsupported features when polyfills are detected');
      } catch (error) {
        assert.fail('Should not throw when polyfills are detected');
      }
  ...
  describe('Module handling', () => {
    it('should parse ES modules correctly', async () => {
      const code = `
        import { foo } from './foo.js';
        export const bar = 1;
      `;

      const { foundFeatures } = await detectFeatures(code, 6, 'module', new Set());
      assert(Object.values(foundFeatures).some(Boolean), 'Should detect some features in ES modules');
    });

    it('should throw for ES modules in script mode', async () => {
      const code = `
        import { foo } from './foo.js';
        export const bar = 1;
      `;
      ...
      try {
        await detectFeatures(code, 6, 'script', new Set());
  ...
  describe('Feature detection', () => {
    it('should not confuse console.group with ArrayGroup', async () => {
      const code = `
      ...
      try {
        const { foundFeatures, unsupportedFeatures } = await detectFeatures(code, 13, 'script', new Set());
    ...
    it('should detect ES2020 features', async () => {
      const code = `
        const obj = {
          prop: 1,
          method() {
      ...
      `;
      const { foundFeatures } = await detectFeatures(code, 2020, 'script', new Set());
      assert(Object.values(foundFeatures).some(Boolean), 'Should detect some ES2020 features');
    });

    it('should detect ES2021 features', async () => {
      const code = `
        const str = 'hello';
        const replaced = str.replaceAll('l', 'x');
      `;
      const { foundFeatures } = await detectFeatures(code, 2021, 'script', new Set());
      assert(Object.values(foundFeatures).some(Boolean), 'Should detect some ES2021 features');
    });

    it('should detect ES2022 features', async () => {
      const code = `
      ...
      const { foundFeatures } = await detectFeatures(code, 2022, 'script', new Set());
