...
function findAllRequiredFiles() {
  ...
  return Array.from(requiredFiles).filter(file => 
...
function verifyPackageFiles() {
  ...
  for (const file of filesInPackage) {
    if (!fs.existsSync(path.join(rootDir, file))) {
      ...
      console.log(`ℹ️  ${file} - in package.json but not detected as dependency`);
    }
  }
  ...
  try {
    ...
    for (const requiredFile of requiredFiles) {
      if (!packedFiles.includes(requiredFile)) {
        console.log(`❌ Required file "${requiredFile}" will NOT be included in npm package`);
        hasError = true;
      }
    }
