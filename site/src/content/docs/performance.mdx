...
As of v9.4.0, ES Check includes several major performance optimizations:

- **Lightweight mode (`--light`)**: NEW! Faster checking using pattern matching instead of full AST parsing
...
_Note: Performance improvements vary based on your specific use case, file sizes, and system configuration._

## Lightweight Mode (`--light`)

**New in v9.4.0!** The `--light` flag enables a fast pattern-matching mode for improved performance:

```bash
# Standard mode (full AST parsing)
...
- Pre-commit hooks that need to run fast

**Trade-offs:**

- Faster execution through pattern matching
- Uses pattern matching instead of full AST parsing
- May have slightly different error messages
- Best for syntax validation rather than detailed feature detection

**Note:** `--light` mode focuses on speed while still catching incompatible syntax. For comprehensive feature detection, use standard mode with `--checkFeatures`.
...
| ------------------------------------- | ----------------- | ------------------------------------------------------------------- | ---------------------------- |
| **--light mode**                      | **Fastest**       | `es-check es5 './dist/**/*.js' --light`                             | Fast CI/CD validation        |
| Syntax only + silent + specific files | **1x** (baseline) | `es-check es5 './dist/bundle.js' --silent`                          | CI/CD quick validation       |
...
```bash
# NEW: Use --light mode for faster checking
es-check es5 './dist/**/*.js' --light --silent --batchSize 0

# Alternative: Syntax only, silent output, specific files
