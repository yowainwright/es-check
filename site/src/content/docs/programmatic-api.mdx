...
async function checkES5Compatibility() {
  ...
  if (result.success) {
    ...
    console.error(`❌ ES5 compatibility check failed with ${result.errors.length} errors`);
...
```javascript
const { runChecks, createLogger } = require("es-check");
...
const config = {
  ...
  checkFeatures: true, // Check ES features and browser support when checkBrowser is enabled
  checkBrowser: false, // Use Browserslist targets; combine with checkFeatures for browser checks
...
Set `checkFeatures: true` with `ecmaVersion: "checkBrowser"` or `checkBrowser: true` to check
detected features against individual target browser versions as well as the resolved ES version.
...
async function checkBrowserCompatibility() {
  ...
  // Debug logging includes the resolved ES version and target browser versions.
  return { success: true };
}
```
...
async function detailedErrorHandling() {
  ...
  if (!result.success) {
    console.error(`ES Check found ${result.errors.length} incompatible files:\n`);
...
```javascript
const detectFeatures = require("es-check/lib/detectFeatures");
...
try {
  ...
  // detectFeatures throws on unsupported features
  console.error("Feature detection error:", error.message);
  console.error("Unsupported features:", error.features);
}
```

**Note:** The `detectFeatures` function throws an error when unsupported features are detected, unlike the main `runChecks` API which returns a result object.
