...
### 3. Check Inline Scripts From HTML

ES Check parses JavaScript and TypeScript files, not full HTML documents. To check JavaScript embedded in an HTML `<script>` tag, extract the script contents to a temporary `.js` file and check that file:

```javascript
let goUrlButton = document.querySelector("#go-url-btn");

goUrlButton?.addEventListener("click", () => {
  window.location.reload();
});
```

```bash
es-check es2015 './tmp/inline-script.js'
```

This reports optional chaining because `?.` is newer than ES2015.
