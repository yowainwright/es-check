...
describe('Polyfill Detection', () => {
  describe('detectPolyfills', () => {
    it('should detect core-js polyfills via import', () => {
      ...
      assert.strictEqual(polyfills.has('Array.prototype.toSorted'), true, 'Should detect Array.prototype.toSorted polyfill');
      assert.strictEqual(polyfills.has('Object.hasOwn'), true, 'Should detect Object.hasOwn polyfill');
    });
    ...
    it('should detect manual polyfill definitions', () => {
      ...
      `;
      const polyfills = detectPolyfills(code, mockLogger, testFeatureMap);
      assert.strictEqual(polyfills.has('Array.prototype.toSorted'), true, 'Should detect manual Array.prototype.toSorted polyfill');
    });
    ...
    it('should detect core-js polyfills via require', () => {
      ...
      const polyfills = detectPolyfills(code, mockLogger, testFeatureMap);
      assert.strictEqual(polyfills.has('Promise'), true, 'Should detect Promise polyfill via require');
    });
    ...
    it('should handle empty or null code inputs gracefully', () => {
      assert.strictEqual(detectPolyfills('', mockLogger).size, 0, 'Should return an empty set for an empty string');
      assert.strictEqual(detectPolyfills(null, mockLogger).size, 0, 'Should return an empty set for null input');
      assert.strictEqual(detectPolyfills(undefined, mockLogger).size, 0, 'Should return an empty set for undefined input');
    });

    it('should handle a missing feature map gracefully', () => {
      const code = `import 'core-js/modules/es.array.to-sorted';`;
      assert.strictEqual(detectPolyfills(code, mockLogger, null).size, 0, 'Should return an empty set for null map');
    });
    ...
    it('should call the logger with detected polyfills when provided', () => {
      ...
      const spyLogger = {
        isLevelEnabled: () => true,
        debug: (message) => { loggedMessage = message; },
      };
